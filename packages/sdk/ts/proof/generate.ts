...
export const generateProofs = async ({
	...
	if (isOutputDirExists) {
		try {
			...
			// Directory might be empty or have permission issues, continue
		}
	}

	if (existingProofs.length > 0 && !incremental) {
		throw new Error(
			`Proof files already exist in ${outputDir}: ${existingProofs.join(', ')}.\n` +
			'Deleting and regenerating proofs will create different commitments due to random salts.\n' +
			'This can cause verification failures when the commitments don\'t match.\n' +
			'Use the --incremental flag to allow regeneration, but be aware this will cause issues ' +
			'if you need consistent commitments for on-chain verification.'
		);
	}

	if (existingProofs.length > 0 && incremental) {
		console.warn(
			'\n⚠️  WARNING: Regenerating existing proofs. This will create different commitments due to random salts.\n' +
			'   Use with caution as this may cause verification issues when submitting on-chain.\n' +
			`   Existing files: ${existingProofs.join(', ')}\n`
		);
	}
